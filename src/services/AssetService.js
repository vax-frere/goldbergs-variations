...
export const ASSET_TYPES = {
  ...
  INSTANCED_MESH: "instanced_mesh", // Nouveau type pour les meshes instanciés
};
...
const useAssetStore = create((set, get) => ({
  ...
  assets: {
    ...
    instancedMeshes: {}, // Nouveau cache pour les meshes instanciés
  },
  loading: {
    inProgress: false,
    total: 0,
    ...
    function onAssetLoaded(id, asset, type) {
      ...
      switch (type) {
        ...
        case ASSET_TYPES.MATERIAL:
          ...
          break;
        case ASSET_TYPES.INSTANCED_MESH:
          newAssets.instancedMeshes[id] = asset;
          break;
      }
  ...
  getAsset: (id, type) => {
    ...
    switch (type) {
      ...
      case ASSET_TYPES.MODEL:
        return assets.models[id] || null;
      case ASSET_TYPES.GEOMETRY:
        return assets.geometries[id] || null;
      case ASSET_TYPES.MATERIAL:
        return assets.materials[id] || null;
      case ASSET_TYPES.INSTANCED_MESH:
        return assets.instancedMeshes[id] || null;
      default:
  ...
  /**
   * Obtenir un mesh instancié par son ID
   * @param {string} id - ID du mesh instancié
   * @returns {THREE.InstancedMesh|null} - Le mesh instancié demandé ou null
   */
  getInstancedMesh: (id) => {
    return get().getAsset(id, ASSET_TYPES.INSTANCED_MESH);
  },
  ...
  /**
   * Crée et stocke un mesh instancié
   * @param {string} id - ID unique pour le mesh instancié
   * @param {Object} options - Options pour la création du mesh instancié
   * @param {string|THREE.BufferGeometry} options.geometry - ID de la géométrie ou géométrie directement
   * @param {string|THREE.Material} options.material - ID du matériau ou matériau directement
   * @param {number} options.count - Nombre d'instances
   * @returns {THREE.InstancedMesh} - Le mesh instancié créé
   */
  createInstancedMesh: (id, options) => {
    // Vérifier si le mesh instancié existe déjà
    const existingMesh = get().getInstancedMesh(id);
    if (existingMesh) {
      return existingMesh;
    }

    try {
      // Récupérer la géométrie (soit par ID, soit directement)
      let geometry;
      if (typeof options.geometry === "string") {
        geometry = get().getGeometry(options.geometry);
        if (!geometry) {
          throw new Error(`Géométrie non trouvée: ${options.geometry}`);
        }
      } else {
        geometry = options.geometry;
      }

      // Récupérer le matériau (soit par ID, soit directement)
      let material;
      if (typeof options.material === "string") {
        material = get().getMaterial(options.material);
        if (!material) {
          throw new Error(`Matériau non trouvé: ${options.material}`);
        }
      } else {
      ...
      // Stocker le mesh instancié
      const newAssets = { ...get().assets };
      newAssets.instancedMeshes[id] = instancedMesh;
      ...
      console.error(
        `[AssetService] Erreur lors de la création du mesh instancié ${id}:`,
        error
      );
      return null;
    }
  ...
  dispose: () => {
    ...
    // Disposer les meshes instanciés
    Object.values(assets.instancedMeshes).forEach((mesh) => {
      if (mesh) {
        ...
        // THREE.InstancedMesh n'a pas de méthode dispose() dédiée
      }
    });
