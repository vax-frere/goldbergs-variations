...
function ShootingStarWithFadingTail({
  ...
  // Créer les matériaux lors du premier rendu
  useEffect(() => {
    // Matériau pour le point principal
    pointMaterialRef.current = new THREE.PointsMaterial({
      size,
      map: texture,
      transparent: true,
      ...
      depthWrite: false,
      color: color,
      opacity: fading ? Math.max(0, 1 - fadeProgress * 2) : 1.0,
      sizeAttenuation: true,
    });

    // Matériau pour la traînée
    tailMaterialRef.current = new THREE.ShaderMaterial({
      uniforms: {
    ...
    // Cleanup
    return () => {
      if (pointMaterialRef.current) pointMaterialRef.current.dispose();
      if (tailMaterialRef.current) tailMaterialRef.current.dispose();
    };
  }, [color, size, texture]);

  // Mettre à jour l'opacité du point principal
  useEffect(() => {
    if (pointMaterialRef.current) {
      pointMaterialRef.current.opacity = fading
  ...
  // Ne rien rendre si les matériaux ne sont pas encore initialisés
  if (!pointMaterialRef.current || !tailMaterialRef.current) {
    return null;
  }
  ...
  return (
    <group renderOrder={100}>
      ...
      <points ref={pointRef} position={[0, 0, 0]} renderOrder={101}>
        ...
        <primitive object={pointMaterialRef.current} attach="material" />
      </points>

      {/* Traînée de l'étoile filante */}
      {trail.length > 1 && (
        <points ref={tailRef} renderOrder={100}>
          <primitive object={tailMaterialRef.current} attach="material" />
        </points>
      )}
    </group>
  );
