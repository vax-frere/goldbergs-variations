...
export class InputManager {
  ...
  pollGamepad() {
    ...
    // Action d'interaction (bouton X ou carré) - même que toggleMode mais géré différemment
    if (gamepad.buttons[2]?.pressed && !this.previousButtonStates.interact) {
      this.gamepadInputs.interact = true;
    } else {
      this.gamepadInputs.interact = false;
    }
    this.previousButtonStates.interact = gamepad.buttons[2]?.pressed;
