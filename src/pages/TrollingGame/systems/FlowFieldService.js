...
export class FlowFieldService {
  constructor(scene, config = {}) {
    ...
    this.config = {
      ...
      weightCenter: config.weightCenter || -0.25, // négatif = centre attractif (potentiel plus bas)
      playerSigma: config.playerSigma || 180,     // échelle gaussienne répulsion joueur (px)
      borderMargin: config.borderMargin || 140,   // marge d'influence des bords (px)
  ...
  buildPotential() {
    ...
    for (let y = 0; y < this.rows; y++) {
      for (let x = 0; x < this.cols; x++) {
        ...
        // Bordures: potentiel élevé proche des bords (barrière douce)
        ...
        const borderP = (dxL + dxR + dyT + dyB) / borderMargin; // [0..~4]

        // Joueur: gaussienne centrée sur le joueur
        const dpx = gx - px;
        const dpy = gy - py;
        const r2 = dpx * dpx + dpy * dpy;
        const playerP = Math.exp(-r2 * invTwoSigma2); // [0..1]

        // Centre: rendre le centre plus bas (attire)
        const dcx = gx - cx;
        const dcy = gy - cy;
        const centerR = Math.sqrt(dcx * dcx + dcy * dcy);
        const maxC = Math.sqrt(cx * cx + cy * cy);
        const centerP = centerR / Math.max(1, maxC); // [0..1], sera pondéré négativement
    ...
    for (let p = 0; p < passes; p++) {
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          let acc = src[idx] * 2;
          let w = 2;
          if (x > 0) { acc += src[this.index(x - 1, y)]; w++; }
          if (x < cols - 1) { acc += src[this.index(x + 1, y)]; w++; }
          tmp[idx] = acc / w;
        }
      }
      // Vertical
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          let acc = tmp[idx] * 2;
          let w = 2;
          if (y > 0) { acc += tmp[this.index(x, y - 1)]; w++; }
          if (y < rows - 1) { acc += tmp[this.index(x, y + 1)]; w++; }
          src[idx] = acc / w;
        }
      }
    }
  ...
  computeGradient() {
    ...
    for (let y = 0; y < rows; y++) {
      for (let x = 0; x < cols; x++) {
        ...
        // Gradient du potentiel: direction d'augmentation; la fuite prendra -grad
  ...
  sampleDirection(px, py) {
    // Retourne la direction de fuite normalisée = -∇P (bilinéaire)
    const w = this.scene.scale.width || 0;
    const h = this.scene.scale.height || 0;
    ...
    const i00 = this.index(x0, y0), i10 = this.index(x1, y0);
    const i01 = this.index(x0, y1), i11 = this.index(x1, y1);
