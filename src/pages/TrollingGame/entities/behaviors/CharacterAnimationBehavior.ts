...
export class CharacterAnimationBehavior {
  ...
  forcedAnimationName: string | null;
  realSpeedPxPerSec: number;
  lastPosition: { x: number; y: number };
  movementBuffer: number[];
  directionBuffer: string[];
  movementThreshold: number;
  idleThreshold: number;
  lastDirectionChange: number;
  directions: string[];
  directionMapping: Record<string, string>;
  directionAngles: Array<{ dir: string; angle: number; range: number }>;
  spritesheetMetadata: any;
  metadataLoaded: boolean;
  _randomizedLoopKeys: Set<string>;
