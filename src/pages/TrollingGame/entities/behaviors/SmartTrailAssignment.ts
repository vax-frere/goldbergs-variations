...
export class SmartTrailAssignment {
  ...
  npcAssignments: Map<any, any>;
  pointCapacities: Map<number, any>;
  reassignmentTimers: Map<any, number>;
  swapTimers: Map<any, number>;
  stats: Record<string, any>;

  constructor(trailBehavior: any, config: Record<string, any> = {}) {
   ...
   */
  getAssignedPoint(npc: any): { x: number; y: number } | null {
   ...
   */
  calculateLanePosition(basePoint: any, npc: any, pointIndex: number): { x: number; y: number } {
    const capacity = this.getPointCapacity(pointIndex);
